...
class Simulator:
    ...
    def set_params(self,
        ...
        # print(f'DEBUG:Checking simulator.workload.obj topolist params in simulator.py: {self.workload_obj.topo_list}')
        ...
        self.partition_obj.set_params(config_obj=self.config_obj,
                                      ...
                                      SET_json_file=self.SET_json_file
                                      )
        #DEBUG:
        # print(f'DEBUG:Checking partition_obj.workload.obj topolist params in simulator.py: {self.partition_obj.workload.topo_list}')
    ...
    def run(self):
        ...
        for layer_id in range(num_layers):
            ...
            if (layer_params[0] in ['conv', 'gemm']):
               ...
               # print(f"DEBUG:checking what is the op_matrix in simulator.py: {op_matrix.shape}")
                if self.verbose:
                    print('SAVING TRACES')
                ...
                this_layer_sim.set_params(config_obj=self.config_obj,
                                      ...
                                      log_top_path=self.top_path,
                                      verbosity=self.verbose)
                this_layer_sim.run_simd_all_parts(operand_matrix=op_matrix, optype = layer_params[2])
                #print(f"Run simd all parts function done for: {layer_params[2]}")
                self.single_layer_objects_list += [this_layer_sim]

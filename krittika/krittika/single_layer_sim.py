...
class SingleLayerSim:
    ...
    def run_compute_all_parts(self):
        ...
        #print(f"DEBUG:checking what is the op_matrix in simulator.py: {ofmap_matrix.shape}")
    ...
    def run_simd_all_parts(self, operand_matrix, optype = 'relu'):
        ...
        #Code for maxpool:
        if optype == 'maxpool':
            #print("entering maxpool")
            #print(f"the workload manager object topo list is")
            pool_w = self.partitioner_obj.workload.topo_list[self.layer_id][3]
            pool_h = self.partitioner_obj.workload.topo_list[self.layer_id][4]
            pool_s = self.partitioner_obj.workload.topo_list[self.layer_id][6]
            # input_w = self.partitioner_obj.workload.topo_list[self.layer_id-1][2]
            # input_h = self.partitioner_obj.workload.topo_list[self.layer_id-1][3]
            # print(f"input_w and input h: {input_w, input_h}")
            # operations_W = (input_w - pool_w) // pool_s + 1
            # operations_H = (input_h - pool_h) // pool_s + 1
            #windows_per_core = math.ceil(num_windows / (self.num_input_part*self.num_filter_part))
            #print(windows_per_core)
            ...
            for inp_part in range(self.num_input_part):
                for filt_part in range(self.num_filter_part):
                    ...
                    #operand_part = operand_matrix[:pool_w,:pool_h]
                    #print(operand_part.shape)
                    #print(operand_matrix.shape)
                    this_part_compute_node = ComputeNode()
                    #print(f"Optype in single layer sim is: {optype}")
                    this_part_compute_node.set_params(config=self.config_obj,
                                                        compute_unit='simd', optype = optype, partition = self.partitioner_obj)

...
class WorkloadManager:
    ...
    def load_arrays_activation(self, row, layer_id):
        ...
        row = row.strip()
        elems = row.split(',')[:]
        func = str(elems[1].strip())
        if func == 'maxpool':
            pool_h = int(elems[2].strip())
            pool_w = int(elems[3].strip())
            pool_c = int(elems[4].strip())
            pool_s = int(elems[5].strip())
            entry = ['activation', layer_id, func, pool_h, pool_w, pool_c, pool_s]

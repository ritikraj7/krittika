...
private:
	...
	len_t external_C;
public:
	Node(const Layer* _l, const Bitset& _prevs, len_t _external_C, bwidth_t width = 0);
	Node(const Node& n) = delete;
	Node(Node&& n)=default;
	const Layer& layer() const;
	const std::string& name() const;
	const Bitset& get_prevs() const;
...
public:
	...
	lid_t add(const Layer* l, const layer_set& prevs={}, bwidth_t width=0, std::vector<InputData> ext_data={});

...
Layer::Layer(const std::string& _name, const fmap_shape& _ifm_shape, const fmap_shape& _ofm_shape)
	:name(_name), ifm_shape(_ifm_shape), ofm_shape(_ofm_shape), bitwidth(8){}
...
TransposeLayer::TransposeLayer(const std::string& _name, const TransposeLayer::Workload& _wl)
	...
	return lwl;}()), wl(_wl){
	fmap_range ifm_range = {{0, ofm_shape.c}, {0, 1}, {0, ofm_shape.h}, {0, ofm_shape.w}};

...
std::vector<bool> SchNode::from_core, SchNode::to_dram;
...
void LNode::searchLayer(){
	...
	*/
	...
	buf_usage = ifm_usage;
	// FIX_NOW: add wgt_is_ifm.
	if(!place_sch.getWgtL().update(wgt_usage)){
		valid = false;
		return;
	}
...
void LNode::add_workload_and_dfs(len_t batch_offset, len_t segment, std::vector<Json::Value>& workload_list) const{
	...
	for(auto part: ofm_parts){
		...
		if(REF_IS_INSTANCE(layert.layer(), ConvLayer)){
			Json::Value weight;
			...
			if(dirp_set.contains(layerno)){
				for(auto prev_part: lnode->get_place_sch().getOfmL()){
					for(len_t prev_batch_offset=0; prev_batch_offset<tot_batch; prev_batch_offset += lnode->num_batch){
						...
						prev_range.c += prev_channel_offset;
						/*if(layerno == 0)
						{
							...
							printf("ofmap_range_from = %d %d %d %d\n",ofmap_range.b.from,ofmap_range.c.from,ofmap_range.h.from,ofmap_range.w.from);
							printf("ofmap_range_to = %d %d %d %d\n",ofmap_range.b.to,ofmap_range.c.to,ofmap_range.h.to,ofmap_range.w.to);
							printf("batch_offset = %d\n\n",batch_offset);
						}*/
						fmap_range intersect = ofmap_range.intersect(prev_range);
						if(intersect.is_empty())
							continue;
						prev_range.c -= prev_channel_offset;
						intersect.c -= prev_channel_offset;
						from_other_core = 1;
						Json::Value ifmap;
						...
						ifmap["channel"].append(prev_channel_offset + intersect.c.from);
						ifmap["channel"].append(prev_channel_offset + intersect.c.to-1);
				...
				int lower_c, upper_c;
				lower_c = std::max(0, (int)ofmap_range.c.from - (int)prev_channel_offset);
				upper_c = std::min((int)node.layer().ofmap_shape().c, (int)ofmap_range.c.to - (int)prev_channel_offset);
				if(lower_c < upper_c){
					...
					ifmap["channel"].append(prev_channel_offset + lower_c);
					ifmap["channel"].append(prev_channel_offset + upper_c-1);
		...
		FOR_BITSET(layerno, next){
			...
			const Bitset& prev_set = node.get_prevs();
		...
		if(REF_IS_INSTANCE(layert.layer(), ConvLayer)){
			...
			weight["transfer_id"] = workload["weight"]["transfer_id"];
			if(batch_offset % batch_size == 0){

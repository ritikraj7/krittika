...
Node::Node(const Layer* _l, const Bitset& _prevs, len_t _external_C, bwidth_t width)
	:l(_l), prevs(_prevs), external_C(_external_C){
...
const Bitset& Node::get_prevs() const{
	return prevs;
}
...
Network::lid_t Network::add(const Layer* l, const layer_set& prevs, bwidth_t width, std::vector<InputData> ext_data){
	// If no prevs indicated, use default_bs.
	bool default_prev = (ext_data.empty() && prevs.empty());
	lid_t last_id=0;
	Bitset prev_layers;
	if(default_prev){
		assert(!layers.empty());
		...
		for(const lid_t& i: prevs) prev_layers.set(i);
	}
	...
	if(layers.size() >= std::numeric_limits<lid_t>::max()){
		throw std::overflow_error("Too many layers! Consider using a larger format for lid_t (perhaps uint16_t?)");
	}
	...
	layers.emplace_back(l, prev_layers, external_C, width);
...
bool Network::has_dep(const Bitset& src, const Bitset& dst) const{
	...
	if(src.count()>3){
		...
		FOR_BITSET(it, dst){
			const Bitset& dst_set=getNode(it).get_prevs();
...
bool Network::is_chain() const{
	for(size_t i=1;i<layers.size();++i){
		if(!layers[i].get_prevs().contains(i-1)) return false;
	}
	return true;
}
